...
public class EncasedPipeAttachmentModel extends BakedModelWrapperWithData {

	private static final ModelProperty<PipeModelData> PIPE_PROPERTY = new ModelProperty<>();
	private boolean ao;
	private final String setName;

	public static EncasedPipeAttachmentModel withAO(BakedModel template, String setName) {
		return new EncasedPipeAttachmentModel(template, true, setName);
	}

	public static EncasedPipeAttachmentModel withoutAO(BakedModel template, String setName) {
		return new EncasedPipeAttachmentModel(template, false, setName);
	}

	public EncasedPipeAttachmentModel(BakedModel template, boolean ao, String setName) {
		super(template);
		this.ao = ao;
	...
	public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
		...
		set.add(EncasedPartialModels.PIPE_CASINGS.get(setName).get().getRenderTypes(state, rand, data));
		...
		if (data.has(PIPE_PROPERTY)) {
			...
			for (Direction d : Iterate.directions) {
				...
				for (ComponentPartials partial : type.partials) {
					ChunkRenderTypeSet attachmentRenderTypeSet = EncasedPartialModels.PIPE_ATTACHMENTS.get(setName).get(partial).get(d)
	...
	private void addQuads(List<BakedQuad> quads, BlockState state, Direction side, RandomSource rand, ModelData data,
		...
		for (Direction d : Iterate.directions) {
			AttachmentTypes type = pipeData.getAttachment(d);
			for (ComponentPartials partial : type.partials) {
				quads.addAll(EncasedPartialModels.PIPE_ATTACHMENTS.get(setName).get(partial)
					.get(d)
					.get()
		...
		if (pipeData.isEncased())
			quads.addAll(EncasedPartialModels.PIPE_CASINGS.get(setName).get()
				.getQuads(state, side, rand, data, renderType));
	}
